...
5. You may want to go to Core/Inc/defaults.hpp and change #define DRIVER_ID to a different value (any you like), especially if you would use more than one device simultaneously and want to be able to discern between them when talking through serial port.
6. Power up the driver and connect it with micro usb port (near the power input socket, slightly lower). Click Run->Run or click the green arrow icon to build and flash the device.
    - You may be prompted to upgrade the stlink firmware. Agree. In the STLinkUpgrade Window click "Open in update mode" and then "Upgrade". Unplug and plug the micro USB cable and try again number 6 in this instruction.
    - you may be prompted to save the files you edited. Agree.
